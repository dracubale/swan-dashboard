...
function gatesInner(arr){
  ...
  const KHACH={ic:'cust',name:'Khách',cls:' key',sub:'Khách đến phòng khám',big:arrived.toLocaleString('vi-VN'),unit:'khách',
    rows:(isng?row('Khách có DT',paying)+row('Khách chỉ TK',zero)+row('Đến không làm',rot)+row('Chi ad / khách đến',arrived?trd(spend/arrived):'—')+row('Chi ad / khách có DT + cọc',chot?trd(spend/chot):'—'):row('Khách mới đến (tháng)',((typeof DATA!=='undefined'&&DATA.llv&&DATA.llv.by_status)?DATA.llv.by_status.khach_moi_den:dennew))+row('Có DT',paying)+row('Chỉ TK',chiTK)+row('Khách rớt (mới)',rotNew)+row('Chi ad / khách đến',arrived?trd(spend/arrived):'—')+row('Chi ad / khách có DT',paying?trd(spend/paying):'—'))+sub2('Chờ thực hiện · cọc')+row('Đã cọc',`${coc} khách · ${tr(dep)}`,1)};
